      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex flex-col space-y-3">
          ...
          >
            ...
            {t.kilometersLabel} <span className="text-red-500 ml-1">*</span>
          </label>
          <input
            type="text"
            id="kilometers"
            ...
            className="min-w-0 flex-1 rounded-xl border border-slate-200/80 bg-white px-5 py-4 text-lg text-slate-900 shadow-sm ring-1 ring-slate-200/40 transition-all placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:ring-offset-2"
            pattern="[0-9.,]*"
            inputMode="numeric"
      ...
      {carData && (
        <>
          ...
          >
            ...
            <div className={`grid grid-cols-1 gap-3 ${rawData?.kilometers ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}`}>
              ...
              {rawData?.kilometers && (
                <div className="premium-card rounded-xl border border-slate-200/50 bg-slate-50/50 p-4 ring-1 ring-slate-200/40">
                  ...
                  <p className="text-lg font-semibold text-slate-900">{rawData.kilometers.toLocaleString(language === 'da' ? 'da-DK' : 'en-GB')} km</p>
                </div>
              )}
            </div>

            <div
              className={`mt-6 rounded-2xl border-2 border-blue-100 bg-gradient-to-br from-white to-blue-50/50 p-8 shadow-lg shadow-blue-900/5 ring-1 ring-blue-50 ${animateCard ? 'animate-value-hero-pulse' : ''}`}
            >
              ...
              <div className="mt-4 flex items-center gap-2 text-sm text-slate-500">
                ...
                <span>
                  Baseret på markedsværdi fra {carData.minPrice.toLocaleString(language === 'da' ? 'da-DK' : 'en-GB')} - {carData.maxPrice.toLocaleString(language === 'da' ? 'da-DK' : 'en-GB')} kr
                </span>
              </div>
            </div>
